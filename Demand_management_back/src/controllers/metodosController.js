const MetodosService = require("../services/metodosService")
...
const updateStatusDraftAcept = async (req,res) =>{
  ...
  try {
    ...
    const data = await MetodosService.updateStatusDraftAcept(req,res);
...
const createRequestAdjustments = async (req,res)=>{
    ...
    try{
      ...
      const result= await MetodosService.createRequestAdjustment(req,res);
      
      return res.status(200).json({
        success: true,
        message: "Comment added and adjusment requiriment created successfully",
        ...result
      });
